...
type NewTaskPageProps = { searchParams: Promise<{ project?: string }> };

export default async function NewTaskPage({ searchParams }: NewTaskPageProps) {
  const { project } = await searchParams;
  const initialProjectId = project && /^\d+$/.test(project) ? Number(project) : undefined;
  return <main className="min-h-screen px-6 py-10 sm:px-10"><div className="mx-auto max-w-5xl">
    ...
    <section className="py-12">
      ...
      <TaskForm mode="create" initialProjectId={initialProjectId} />
    </section>
  </div></main>;
}
